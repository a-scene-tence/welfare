...
GitHub Actions 워크플로 4개로 운영 위생을 자동화.

| 워크플로 | 트리거 | 역할 | 실패 시 알림 |
|---|---|---|---|
| `ci.yml` | PR / push (모든 branch) | typecheck + vitest + build + **kb:audit** | PR check fail (merge 차단) |
| `e2e.yml` | nightly 04:00 KST + `workflow_dispatch` | Playwright P1·P2·P3 회귀 metric | nightly 실패 시 `e2e-failure` label issue 자동 |
| `kb-audit.yml` | 매주 월 09:00 KST + `workflow_dispatch` | KB `staleAfter` 갱신 임박/만료 점검 | warning 또는 expired 시 `kb-audit` label issue 자동 |
| `health.yml` | 매일 09:00 KST + `workflow_dispatch` | `${{ vars.PRODUCTION_URL }}/api/health` curl 200 검증 | 실패 시 `health-failure` label issue 자동 |
...
2. **Repository Secrets** (Settings → Secrets and variables → Actions → Secrets):
   - `UPSTAGE_API_KEY`, `TAVILY_API_KEY` (E2E 가 실 LLM 호출에 사용)
3. **`pnpm kb:audit` 로컬 실행** — 로컬에서도 동일 결과 확인 가능.

### 11.2 Issue 자동 알림 동작

- 동일 label 의 open issue 가 있으면 comment 만 추가 (스팸 방지).
- 없으면 새 issue 생성. KB 갱신·페르소나 fix·헬스 정상화 후 사용자가 수동 close.
- 모든 알림은 GitHub Issues 에만 — 외부 채널(Slack/Discord) 미사용.

### 11.3 KB audit 동작

- `expired` (staleAfter 가 오늘 이전) ≥ 1 → CI 빨간색 + 주간 cron 의 issue 생성 (fail-on-expired).
- `warning` (staleAfter ≤ 30일) ≥ 1 → CI 통과 (운영 흐름 비차단) + 주간 cron 의 issue 생성.
