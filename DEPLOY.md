...
## 11. 회귀 테스트 연계 · 검증 워크플로

GitHub Actions 워크플로 3개. **PR gate(`ci.yml`)만 자동**, 나머지 둘은 **수동 트리거(`workflow_dispatch`) 전용** — cron 자동 스케줄링은 운영하지 않습니다.

| 워크플로 | 트리거 | 역할 | 실패 시 알림 |
|---|---|---|---|
| `ci.yml` | PR / push (모든 branch) | typecheck + vitest + build + **kb:audit** | PR check fail (merge 차단) |
| `kb-audit.yml` | `workflow_dispatch` (수동) | KB `staleAfter` 갱신 임박/만료 점검 | warning 또는 expired 시 `kb-audit` label issue 자동 |
| `health.yml` | `workflow_dispatch` (수동) | `${{ vars.PRODUCTION_URL }}/api/health` curl 200 검증 | 실패 시 `health-failure` label issue 자동 |

E2E (Playwright P1·P2·P3 회귀) 도 워크플로 없이 `pnpm test:e2e` 로 로컬 수동 회귀 검증만 유지합니다.

### 11.1 사용자 액션 (1회 셋업)

1. **Repository Variables** (Settings → Secrets and variables → Actions → Variables):
   - `PRODUCTION_URL` = `https://<your-vercel-domain>` (예: `https://welfare.vercel.app`)
     - 미설정 시 `health.yml` 은 warning 만 출력하고 skip.
2. **`pnpm kb:audit` 로컬 실행** — 로컬에서도 동일 결과 확인 가능.
3. **수동 실행 방법**: GitHub → Actions → 해당 워크플로 선택 → "Run workflow" 버튼 → 브랜치(`main`) 선택 후 실행.

### 11.2 Issue 자동 알림 동작

- 수동 실행 시점에만 알림이 발생 (자동 cron 알림 없음).
- 동일 label 의 open issue 가 있으면 comment 만 추가 (스팸 방지).
- 없으면 새 issue 생성. KB 갱신·헬스 정상화 후 사용자가 수동 close.
- 모든 알림은 GitHub Issues 에만 — 외부 채널(Slack/Discord) 미사용.

### 11.3 KB audit 동작

- `expired` (staleAfter 가 오늘 이전) ≥ 1 → CI 빨간색 (PR merge 차단) + 수동 실행 시 issue 생성 (fail-on-expired).
- `warning` (staleAfter ≤ 30일) ≥ 1 → CI 통과 (운영 흐름 비차단) + 수동 실행 시 issue 생성.
- `missing fields` (staleAfter 누락) → warning 동일.
...
`welfare_consent_log` 의 30일 TTL 자동 정리.

1. **활성**: Supabase Dashboard → Database → Extensions → `pg_cron` 검색 → Enable.
2. **스케줄 등록**: SQL Editor → [`db/schema.sql`](db/schema.sql) §3 의 두 라인 주석 해제 후 그 블록 (또는 schema.sql 전체) 재실행:
   ```sql
   ...
   ```
3. **확인**: SQL Editor → [`db/verify-cron.sql`](db/verify-cron.sql) 전체 실행.
   - §1 결과에 1행 (`pg_cron`) → 확장 활성됨.
   - §2 결과에 1행 (jobname=`cleanup_welfare_consent_log`) → 스케줄 등록됨.
   - §3 결과는 24h 이내 1건 이상 `succeeded` 가 정상.
