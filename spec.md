...
> 마지막 업데이트: 2026-05-05 (v6: 1.6단계 — 자격 매칭·자체점검 leak·계층 분류·markdown 링크 강제)
...
11. **서버 측 후처리 검증** (`src/lib/regionMatch.ts` + `src/app/api/chat/route.ts`):
    ...
    - 응답 본문에서 화이트리스트 도메인 URL 만 추출해 citations 재구성 (Tavily 자동 emit
      제거 — 무관 지역 URL 차단).
    - 사업 「출처」·「신청 경로」는 markdown 링크 `[제목](URL)` 형식 필수.
12. **자체 점검 leak 차단**: LLM 의 자체 점검 체크리스트와 결과는 내부 검토용으로만
