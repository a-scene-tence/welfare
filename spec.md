...
> 마지막 업데이트: 2026-05-08 (v8: KB 보강 Phase A — 광역시도 6개 30개 사업 추가)
...
```ts
{
  ...
  age: 0~120,
  household: { size: 1~10, annualIncomeKRW },
  marital: { status: "single" } | { status: "married", spouseAge, spouseAnnualIncomeKRW },
  children: [{ age }],
  situations: [enum 13종],
...
## 2. 추천 복지 혜택
   ...
   ### ② 광역시도 사업 ({사용자 시·도})
   ### ③ 기초자치단체 사업 ({사용자 시·군·구})
   각 사업: **사업명**(소관 부처) · 자격 · 혜택(검색 결과 본문에 명시된 정확 수치) · 신청시기 · 구비서류 · 신청경로 · 출처 URL
## 3. 다음 단계 (마감 임박 순)
> 면책 (자동 첨부)
```
...
2. **3-소스 교차검증 + 시행 연도 + URL 실존**: 현재/직전 연도 시행 자료 존재, 자격·금액
   본문 명시, 인용 URL 이 web_search 결과에 그대로 존재 — 셋 중 하나라도 부족 시 사업 삭제
   또는 "확인 보류" 처리.
3. **환각 방지 (5개 절대 금지)**: ① 사업명 본문 언급 vs 출처 누락 ② URL 합성·변형 ③ 다른
   지역 사업의 거주지 일반화 ④ 시행 연도 미확인 단정 ⑤ 연도 표기 임의 작성.
4. **자격 판정 + 프로필 일치 검증**: 기준 중위소득(월 단위 고시) 기본 사용 (§6.1 환산).
   추가로 나이·가구 유형·소득·특수 상황·거주지 5차원 대조 — 한 차원이라도 명백히
   불일치하면 사업 안내 금지 (예: 청년 사업 19~34세 를 65세에게, 신혼부부 사업을
   미혼/1인가구에게 안내 금지).
5. **3계층 + 거주지 도메인 검증**: ② 광역시도 사업은 광역 공식 도메인 출처 필수,
   ③ 시·군·구 사업은 거주 시·군·구 명칭(한글/로마자)이 URL 호스트 또는 경로에 포함된
   출처 필수. 없으면 "확인된 사업 없음" 명시. 다른 지역 사업명 일반화 절대 금지.
6. **최신성 + 검색 쿼리 형식**: 계층별 쿼리 템플릿 강제. ① `<사업명> <연도>`,
   ② `<광역> <사업명> <연도>`, ③ `<광역> <시·군·구> <사업명> <연도>` (시·군·구 키워드 누락 금지).
7. 다각도 키워드 검색
8. 의료·법률·금융 단정 금지
9. **응답 작성 직전 자체 점검**: URL 실존, 거주 광역·시군구 도메인 출처 존재, 시행 연도
   표기 정확성, 일반화 표현 부재를 한 번 더 확인 후 출력.
10. **계층 분류 규칙**: 보건복지부·고용노동부·국토교통부·금융위원회 등 중앙부처가
    운영·법령 근거를 가진 전국 공통 사업 (기초연금·노인맞춤돌봄·노인일자리·국민취업지원·
    청년도약계좌·버팀목·디딤돌·아동수당 등) 은 ① 중앙정부 에만 안내. ② ③ 중복 분류 금지.
11. **서버 측 후처리 검증** (`src/lib/regionMatch.ts` + `src/lib/centralPrograms.ts` +
    `src/lib/sourceLinkFix.ts` + `src/app/api/chat/route.ts`):
    - 응답 스트림 종료 후 ② ③ 계층 블록 추출 → 인용 URL 이 거주지 도메인 매칭하는지
      deterministic 검증 → 미통과 시 "⚠ 자동 검증 경고" 자동 첨부.
      `tierBlockIsEmpty` 는 블록 첫 300자만 검사 (메타 메시지 leak false match 차단).
    - **계층 중복 분류 검출**: ② ③ 블록에 중앙부처 사업명 키워드(기초연금·노인맞춤돌봄·
      노인일자리·청년도약계좌·버팀목·디딤돌·근로장려금·국민기초생활보장 등)가 등장하면
      "⚠ 계층 중복 분류" 경고 자동 첨부 (`findCentralProgramsInBlock`).
    - **markdown 링크 본문 후처리**: "출처: 복지로 · 국토교통부" 처럼 도메인 텍스트만
      적힌 행을 본문 URL 과 매칭해 `[복지로](URL) · [국토교통부](URL)` 로 자동 재작성
      (`fixSourceLinks`). 변환 결과는 SSE done 이벤트의 `finalMarkdown` 필드로 전송,
      클라이언트 `ChatStream.tsx` 가 누적 텍스트를 통째로 교체.
    - 응답 본문에서 화이트리스트 도메인 URL 만 추출해 citations 재구성 (Tavily 자동 emit
      제거 — 무관 지역 URL 차단).
12. **시뮬레이션 표 금지**: "예상 수혜 금액 시뮬레이션" 섹션 자체를 출력 형식에서
    제거. 사업별 「혜택」 행에는 검색 결과 본문에 명시된 정확 수치만 1줄 표기.
    페르소나 회귀에서 시뮬레이션 표가 환각 수치(근로장려금 연 600~1,200만원, 청년도약계좌
    3년 만기 2,197만원 등)를 만들어내는 것을 차단.
13. **자체 점검 leak 차단**: LLM 의 자체 점검 체크리스트와 결과는 내부 검토용으로만
    수행, 응답 본문에 출력 금지. 메타 경고는 서버가 자동 첨부.
14. 응답 말미 면책 자동 첨부

### 6.1 소득 기준 단위 환산

폼 입력은 **본인·배우자 연 총급여(원, 세전)**. `buildUserPrompt.ts` 가 가구합산 연 총급여와
÷12 월 환산 값을 모두 계산해 LLM에 제공. 사업별 자격 기준 단위에 따라:

| 사업 자격 기준 | LLM 처리 |
|---|---|
| 기준 중위소득 % (월, 가구합산 세전) | 프로필의 「가구합산 월 환산 소득(÷12)」 와 비교 (기초생활·청년월세·한부모·주거급여 등 다수) |
| 연 총급여 (근로소득자) | 프로필의 「가구합산 연 총급여」 또는 본인 연 총급여를 그대로 비교 (예: 청년도약계좌 개인 ≤ 7,500만원) |
| 연 종합소득금액 | 환산 부정확 → 「홈택스 또는 129 문의 권장」 |
| 건강보험료 (월 부과액) | 직장/지역 차이로 추정값임을 명시 |

LLM은 응답 시 사업의 기준 단위를 식별한 뒤 위 절차로 비교하고, 모든 자격 판정에
"추정"임을 명시합니다.

## 7. 데이터 소스

### 7.1 시드 KB (정적, 중앙 26개 + 광역 30개)

3계층 디렉터리:
- `src/data/programs/central/*.md` — 26개 중앙정부 사업
- `src/data/programs/regional/{sido}/*.md` — 광역시도 자체 사업 (Phase A: 6개 광역 × 5 = 30개)
- `src/data/programs/local/{sido-sigungu}/*.md` — 기초자치단체 자체 사업 (Phase C 예정)
...
| `region.{sido,sigungu}` | 광역(sigungu null) 또는 기초자치단체 사업 (중앙은 미기재) |
...
`src/lib/kb/loader.ts` 가 `loadAllPrograms()` 로 3계층 통합 로드. `filterPrograms`
가 거주지 sido/sigungu 매칭 + 정렬 우선순위(기초 > 광역 > 중앙) 적용.

**중앙 사업 (26)**: 기초연금·국민기초생활보장·청년월세·버팀목·디딤돌·청년도약계좌·
한부모가족·부모급여/아동수당·실업급여·장애인연금·노인장기요양·노인맞춤돌봄·노인일자리·
국민행복카드·다문화·국민건강보험·근로/자녀장려금·에너지바우처·주거급여·신혼주거·
국민취업제도·보훈·북한이탈주민·농어업·국가장학·아이돌봄.

**광역 Phase A (30)**: 서울·경기·부산·인천·광주·대구 각 5개. 청년·자립/주거/자산
형성·신혼부부 주거·노인 일자리·돌봄·1인가구·출산축하 등 도메인 균형.

후속 Phase B(나머지 광역 11개)·Phase C(인구 상위 시·군·구 ~110개) 는 plan §21 에 청사진.
