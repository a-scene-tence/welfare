...
> 마지막 업데이트: 2026-05-05 (v5: 환각 방지 1.5단계 — 서버 측 후처리 검증 + citation 본문 추출)
...
4. 자격 판정: **기준 중위소득(월 단위 고시)** 을 기본으로 사용. 사업별 단위가 다르면
   환산 (§6.1).
...
10. **서버 측 후처리 검증** (`src/lib/regionMatch.ts` + `src/app/api/chat/route.ts`):
