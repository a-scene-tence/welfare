...
> 마지막 업데이트: 2026-05-04 (v2: 소득 단위 환산 정책)
...
폼 입력은 **본인·배우자 월 세전 소득(원)**. 사업별 자격 기준 단위에 따라:

| 사업 자격 기준 | 처리 |
|---|---|
| 기준 중위소득 % (월, 가구합산 세전) | 입력 그대로 비교 (기초생활·청년월세·한부모·주거급여 등 다수) |
| 연 총급여 (근로소득자) | 월 × 12 환산 (예: 청년도약계좌 개인 ≤ 7,500만원 → 월 ≤ 625만원) |
| 연 종합소득금액 | 환산 부정확 → 「홈택스 또는 129 문의 권장」 |
| 건강보험료 (월 부과액) | 직장/지역 차이로 추정값임을 명시 |

LLM은 응답 시 사업의 기준 단위를 식별한 뒤 위 절차로 환산하고, 모든 자격 판정에
"추정"임을 명시합니다.
