...
- (선택) Supabase — opt-in 로그 저장 (Postgres + RLS, 30일 TTL)
...
1. `pnpm install` → `cp .env.example .env.local` → 공급자 키 입력 (Upstage 기본: `UPSTAGE_API_KEY` + `TAVILY_API_KEY`)
2. `pnpm dev` 로컬 검증
3. `git push -u origin claude/welfare-benefits-agent-RyIee`
4. Vercel: New Project → Import → ENV 입력 → Region `icn1` → Deploy
5. (선택) Supabase 통합 → `db/schema.sql` 실행 → `SUPABASE_URL`+`SUPABASE_SERVICE_ROLE_KEY` 환경변수 입력 (절차: `db/README.md`)
...
- 다음:
  ...
  - [x] Supabase 마이그레이션 SQL (`db/schema.sql`) + logger 통합
