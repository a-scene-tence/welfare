...
>
> 마지막 업데이트: 2026-05-04 (v2: 소득 입력 step 버그·환산 정책)
...
### [2026-05-04] 정책 정합성 — 한국 복지 소득 기준 단위 다양성

- **증상**: 사용자가 "월 세전 vs 연말정산 총급여 기준" 의 정합성 의문 제기.
- **사실**: 보건복지부 「기준 중위소득」 자체는 **월 단위 고시** → 폼의 "월 세전"
  입력은 다수 사업과 정합. 단, 일부 사업은 다른 기준 사용:
  - 연 총급여 (근로자): 청년도약계좌 (개인 ≤ 7,500만원 등)
  - 연 종합소득금액: 근로/자녀 장려금
  - 월 건강보험료: 일부 보육·바우처
- **현재 처리** (2026-05-04 갱신):
  - UI: `IncomeInput.tsx` 에 헬프 텍스트로 "기준 중위소득은 월 단위, 연 단위 사업은 ×12 환산"
    명시.
  - 시스템 프롬프트: §4) 자격 판정 섹션을 단위별 환산 규칙 표로 교체. 종합소득금액은
    환산 부정확 → 「홈택스 또는 129 문의 권장」 처리.
- **재발 방지**:
  - 새 KB 사업 추가 시 frontmatter 또는 본문에 자격 기준의 단위 명시 (월 가구합산 % /
    연 총급여 / 연 종합소득금액 / 건강보험료 등).
  - 페르소나 회귀 검증 시 청년도약계좌·근로장려금이 ×12 환산을 적용하는지 확인.
